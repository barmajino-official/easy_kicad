...
from barmajinokad.easyeda.parameters_easyeda import Ee3dModel
from barmajinokad.kicad.parameters_kicad_footprint import Ki3dModel

VRML_HEADER = """#VRML V2.0 utf8
# 3D model generated by barmajinokad.py (https://github.com/uPesy/barmajinokad.py)
"""
...
class Exporter3dModelKicad:
    ...
    def export(self, lib_path: str) -> None:
        name = self.output.name if self.output else (self.input.name if self.input else "model")
        if self.output:
            with open(
                file=f"{lib_path}.3dshapes/{name}.wrl",
                mode="w",
                encoding="utf-8",
            ) as my_lib:
        ...
        if self.output_step:
            with open(
                file=f"{lib_path}.3dshapes/{name}.step",
                mode="wb",
            ) as my_lib:
